	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				//шукаємо найвигіднішу ціну для покупця
				if ($product_special_query->num_rows && $sales == 0) {
					if($product_special_query->num_rows > 1){
						foreach ($product_special_query->rows as $psq) {
							if(isset($tmp_p)){
								if($tmp_p > $psq['price']){
									$tmp_p = $psq['price'];
								}
