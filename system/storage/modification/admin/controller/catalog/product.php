...
class ControllerCatalogProduct extends Controller {
	private $error = array();
			...
			private function getCustomerGroupPrices($product_prices, $customer_groups)
			{
				...
				foreach($customer_groups as $customer_group)
				{
					$customer_group_prices[] = array(
						...
						'price' => $this->getCustomerGroupPrice($product_prices, $customer_group['customer_group_id'])
					);
				}
			...
			private function getCustomerGroupPrice($product_prices, $customer_group_id)
			{
				foreach($product_prices as $product_price)
				{
					if($product_price['customer_group_id'] == $customer_group_id					
					&& (int)$product_price['priority'] == 999
					&& ($product_price['date_start'] == '0000-00-00' || $product_price['date_start'] == '')
					&& ($product_price['date_end'] == '0000-00-00' || $product_price['date_end'] == ''))
					{
...
$data['button_apply'] = $this->language->get('button_apply');
		...
		$data['help_sales'] = $this->language->get('help_sales');
    ...
    // OCFilter end
		...
		$data['product_relateds'] = array();
			...
			$this->setDataLang($data, 'entry_customer_group_price');
			$this->setDataLang($data, 'entry_customer_group_discount');
			$this->setDataLang($data, 'entry_customer_group_special');
			$this->setDataLang($data, 'text_customer_group_price_help');
			
			if (isset($this->request->post['customer_group_discounts'])) {
				$data['customer_group_prices'] = $this->request->post['customer_group_discounts'];
			} else {
				if(isset($data['product_discounts']))
				{
					$data['customer_group_discounts'] = $this->getCustomerGroupPrices($data['product_discounts'], $data['customer_groups']);
				}
				else
					$data['customer_group_discounts'] = array();
			}
			
			if (isset($this->request->post['customer_group_specials'])) {
				$data['customer_group_specials'] = $this->request->post['customer_group_specials'];
			} else {
				if(isset($data['product_specials']))
				{
					$data['customer_group_specials'] = $this->getCustomerGroupPrices($data['product_specials'], $data['customer_groups']);
				}
				else
					$data['customer_group_specials'] = array();
			}
			
			if(isset($data['product_discounts']))
			{
				$temp = $data['product_discounts'];
				$data['product_discounts'] = array();
				
				foreach($temp as $id => $product_discount)
				{
					if((int)$product_discount['quantity'] == 1
					&& (int)$product_discount['priority'] == 999
					&& ($product_discount['date_start'] == '' || $product_discount['date_start'] == '0000-00-00')
					&& ($product_discount['date_end'] == '' || $product_discount['date_end'] == '0000-00-00'))
					{
						//skip this discount
					}
					else
					{
						$data['product_discounts'][] = $product_discount;
					}
				}
			}
			...
			if(isset($data['product_specials']))
			{
				$temp = $data['product_specials'];
				$data['product_specials'] = array();
				
				foreach($temp as $id => $product_special)
				{
					if((int)$product_special['priority'] == 999
					&& ($product_special['date_start'] == '' || $product_special['date_start'] == '0000-00-00')
					&& ($product_special['date_end'] == '' || $product_special['date_end'] == '0000-00-00'))
					{
						//skip this discount
					}
					else
					{
						$data['product_specials'][] = $product_special;
					}
				}
			}
			//EOF Customer Group Price
