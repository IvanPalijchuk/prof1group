...
$data['button_not'] = $this->language->get('button_not');
		...
		if ($data['items']) {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/module/showcase/' . $data['subitems_pos'])) {
				return $this->load->view($this->config->get('config_template') . '/template/extension/module/showcase/' . $data['subitems_pos'], $data);
			} else {
				return $this->load->view('extension/module/showcase/' . $data['subitems_pos'], $data);
			}
		}
	}
}