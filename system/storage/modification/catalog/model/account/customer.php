...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$number_phone = preg_replace("/[^0-9]/", '', $data['telephone']);
		$number_phone = substr($number_phone, -9);
		$customer_gr_id = $this->getIdUserGroupDiscount($number_phone);
		if(!empty($customer_gr_id)){
			$customer_group_id = $customer_gr_id;
		}
	...
	public function getIdUserGroupDiscount($number_phone){
		$user_id = $this->db->query("SELECT customer_group_id FROM " . DB_PREFIX . "user_discount WHERE phone LIKE '%".$number_phone."'");
		if($user_id->num_rows > 0){
			return $user_id->row['customer_group_id'];
		}
	}
	...
	public function getLoginTele($email) {
		$query = $this->db->query("SELECT email FROM `" . DB_PREFIX . "customer` WHERE telephone = '" . $this->db->escape(utf8_strtolower($email)) . "'");
		if($query->row){
