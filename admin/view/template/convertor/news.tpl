      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="form">
		 ...
		 <tr>
		   ...
		   <td>
			   <select id="category_news_id" name="category_news">
				<?php foreach ($categories as $cat) {  ?>
			     	<option value="<?php echo $cat['blog_id']; ?>" <?php if (isset($category_news) && $category_news == $cat['blog_id']) { ?> selected="selected" <?php } ?>><?php echo $cat['name']; ?></option>
			   	<?php } ?>
			    </select>
		     </td>
		 </tr>
...
<a onclick="
        var file_sitemap = $('#category_news_id').val();
 		$.ajax({
					url: '<?php echo $url_convert_news; ?>',
					type: 'post',
					data:  {category_news_id: file_sitemap },
					dataType: 'html',
					beforeSend: function()
					{
                      $('.genbutton_').hide();
