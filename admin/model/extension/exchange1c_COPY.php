    ...
    private function updateProduct($product_id, $data) {
        ...
        */
        ...
        if ($data['description'] != ""){
            ...
            $this->log("~CCL_update_data");
            $this->log($this->db->escape($data['name']));
            $this->log($this->db->escape($data["description"]));
            $this->log($product_id);
            $this->log($ccl_lang_id_ru_ok);
        ...
        if ($data['naim_eng'] != ""){
            ...
            $this->log("~CCL_uaname_descr");
            $this->log($data['naim_eng']);
            $this->log($data["description_en"]);

            $this->query("UPDATE `" . DB_PREFIX . "product_description` SET  `name` ='" . $this->db->escape($data['naim_eng']) . "' , `description` ='".$this->db->escape($data["description_en"])."' WHERE `product_id` = " . (int)$product_id." AND  `language_id` = ".$ccl_lang_id_en_ok);
        }
...
//              }
     ...
     */
    private function setProductOptions($data_options, $product_id, $product_options, $product_feature_id, $new_price, $quantity = 0, $price = 0) {
