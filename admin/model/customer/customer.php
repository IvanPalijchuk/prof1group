...
class ModelCustomerCustomer extends Model {
	...
	public function getIdGroup($name_group){
		$group_id = $this->db->query("SELECT customer_group_id FROM " . DB_PREFIX . "customer_group_description WHERE name = '" . $this->db->escape($name_group) . "' LIMIT 1");
		if($group_id->num_rows == 0){
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_group_description SET customer_group_id = '".$group_id."', language_id = '1', name = '".$this->db->escape($name_group)."'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_group_description SET customer_group_id = '".$group_id."', language_id = '2', name = '".$this->db->escape($name_group)."'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_group_description SET customer_group_id = '".$group_id."', language_id = '3', name = '".$this->db->escape($name_group)."'");
			return $group_id;
		}else{
			return $group_id->row['customer_group_id'];
		}
	}
	public function getIdCustomer($number_phone, $customer_group_id){
		$user_id = $this->db->query("SELECT customer_id FROM " . DB_PREFIX . "customer WHERE telephone LIKE '%".$number_phone."'");
		if($user_id->num_rows > 0){
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "' WHERE customer_id = '" . (int)$user_id->row['customer_id'] . "'");
			//echo '<pre>';print_r($user_id->row['customer_id']);echo '<pre>';die('die');
		}
	}
	public function getIdManufacture($name_manufacture){
		$man_id = $this->db->query("SELECT manufacturer_id FROM " . DB_PREFIX . "manufacturer WHERE name = '".$name_manufacture."'");
		if($man_id->num_rows > 0){
			return $man_id->row['manufacturer_id'];
		}
	}
	public function getProductsManufacture($manufacture_id){
		$query = $this->db->query("SELECT product_id, price FROM " . DB_PREFIX . "product WHERE manufacturer_id = '".$manufacture_id."'");
		return $query->rows;
	}
	public function setPriceDiscountProduct($product_id, $customer_group, $product_price_old, $product_price){
		//спочатку видаляємо старі при наявності дані
		$query = $this->db->query("SELECT product_discount_id FROM " . DB_PREFIX . "product_discount WHERE product_id = '".(int)$product_id."' AND customer_group_id = '".(int)$customer_group."'");
		if($query->num_rows > 0){
			$product_discount = $query->row['product_discount_id'];
			if(!empty($product_discount)){
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_discount WHERE product_discount_id = '" . (int)$product_discount . "'");
			}
		}
		if($customer_group > 0){
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_discount SET product_id = '" . (int)$product_id . "', customer_group_id = '" . (int)$customer_group . "', price = '" . (float)$product_price_old . "', quantity = '1', priority = '999'");
		}
		$query = $this->db->query("SELECT product_special_id FROM " . DB_PREFIX . "product_special WHERE product_id = '".(int)$product_id."' AND customer_group_id = '".(int)$customer_group."'");
		if($query->num_rows > 0){
			$product_special = $query->row['product_special_id'];
			if(!empty($product_special)){
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_special_id = '" . (int)$product_special . "'");
			}
		}
		if($customer_group > 0){
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', customer_group_id = '" . (int)$customer_group . "', price = '" . (float)$product_price . "', priority = '999'");	
		}	
	}
