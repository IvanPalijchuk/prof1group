...
if (!class_exists('ControllerModuleBlog', false)) {
	class ControllerModuleBlog extends Controller
	{
		...
		public function index($arg = array()) {
			...
			$url_end = getSCQueryString($this->data['get_original'], array('route', '_route_', 'ajax_file', 'prefix', 'wpage', 'cmswidget', 'wsort', 'worder', 'wlimit', $get_pagination));
		...
		protected function ajax_file() {
			...
			try {
				...
				$filename = preg_replace('/[^a-zA-Z0-9\.\-\s+]/', '', html_entity_decode(base64_decode($this->db->escape($this->request->get['ajax_file'])), ENT_QUOTES, 'UTF-8'));
		...
		protected function ajax_write($data) {
			...
			try {
				$this->data = $data;
				$html_name = 'cache.ajax.' . md5($this->data['href_url'] . serialize($this->data['customer_intersect']) . serialize($this->data['userLogged'])) . "." . $this->data['cmswidget'] . "." . $this->config->get('config_store_id') . "." . $this->config->get('config_language_id') . ".tpl";
				$html_name = preg_replace('/[^a-zA-Z0-9\.\-\s+]/', '', html_entity_decode($html_name, ENT_QUOTES, 'UTF-8'));
				$file = DIR_CACHE . $html_name;
			...
			if (isset($this->data['settings_general']['seocmscss_status']) && $this->data['settings_general']['seocmscss_status']) {
				...
				if (file_exists($css_dir . $css_name)) {
							...
							if (SC_VERSION < 20) {
								...
								$css_content = $this->load->view($this->template, $this->data);
