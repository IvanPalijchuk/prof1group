...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	public function index(){
		if ($this->config->get('yandex_market_status')) {
			...
			$allowed_manufacturer = $this->config->get('yandex_market_manufacturer');
			if ((empty($allowed_categories) && empty($allowed_manufacturer))) exit();
			...
			$yml .= '<yml_catalog date="'.date("Y-m-d H:i:s").'">';
			$yml .= '<shop>';
			$yml .= '<company>'.$this->config->get('yandex_market_shopname').'</company>';
			$yml .= '<platform>'.$this->config->get('yandex_market_shopname').'</platform>';
			...
			foreach ($this->currencies as $value) {
				$yml .= '<currency id="'.$value['id'].'" rate="'.$value['rate'].'"/>';
			}
			$yml .= '</currencies>';
			...
			foreach ($this->categories as $key => $value) {
				if(!isset($value['parentId'])){
					$yml .= '<category id="'.$value['id'].'"><![CDATA['.$value['name'].']]></category>';
				}else{
					$yml .= '<category id="'.$value['id'].'" parentId="'.$value['parentId'].'"><![CDATA['.$value['name'].']]></category>';
				}
			...
			$products = $this->model_extension_feed_yandex_market->getProduct($allowed_categories, $allowed_manufacturer, $out_of_stock_id, $vendor_required);
...
//				$data['barcode'] = $product['sku'];
				if ($product['image']) {
					$data['picture'] = 'https://prof1group.ua/image/'.$product['image'];
				}
			...
			foreach ($this->offers as $key => $value) {
				//витягуємо додаткові картинки
				$yml .= '<offer id="'.$value['id'].'" available="'.$value['available'].'">';
				$yml .= '<name><![CDATA['.$value['data']['name'].']]></name>';
				$yml .= '<categoryId>'.$value['data']['categoryId'].'</categoryId>';
				$yml .= '<price>'.$value['data']['price'].'</price>';
				$yml .= '<currencyId>'.$value['data']['currencyId'].'</currencyId>';
				$yml .= '<vendor><![CDATA['.$value['data']['vendor'].']]></vendor>';
				$yml .= '<vendorCode>'.$value['data']['vendorCode'].'</vendorCode>';
				$yml .= '<description><![CDATA['.$value['data']['description'].']]></description>';
				if($value['available'] == 'true'){
					$yml .= '<available>'.$value['available'].'</available>';
				}else{
					$yml .= '<available></available>';
				}
				$picture = array();
				$images = $this->model_extension_feed_yandex_market->getImagesProduct($value['id']);
				if(!empty($images)){
					foreach ($images as $value) {
						$picture[] = 'https://prof1group.ua/image/'.$value['image'];

					}

				}
				if(isset($value['data']['picture'])&&!empty($value['data']['picture'])){
					$yml .= '<picture>'.$value['data']['picture'].'</picture>';
				}
				if(!empty($picture)){
...
//echo trim($yml);
			...
			$this->response->setOutput(trim($yml));
		}
	}
